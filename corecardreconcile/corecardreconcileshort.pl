...
use warnings;
use strict;
use lib 'C:\\Users\\sphillips\\Downloads\\lib\\'; #thanks windows
...
#			print "</table>";
#			print "<p><b>Count of Unmatched transactions in GL for ". cutzeroes($lastdate) . ":\t$dateglcount, Total Amount: $dateccamount</b></p>";
			...
			print "</table>";
			print "<h2>Count of Unmatched transactions in CoreCard for " . cutzeroes($lastdate) .":\t$datecccount, Total Amount: $dateccamount</h2>\n";
			$dateccamount = 0;
		}
		...
		$dateglamount = 0;
		$datecccount = 0;
#		print "<h2>List of Unmatched Transactions in the GL for ".cutzeroes($lastdate).":</h2>\n";
#		print "<table><tr><td>Date<td>Comment<td>Amount</tr>";
	...
	my $count =0;
#	print "<tr><td>$glrec->{POSTDATE}<td>$glrec->{COMMENT}<td>$glrec->{AMOUNT}<tr><td>Suggestions<td colspan=3><table><tr><td>Date<td>Time<td>Card<td>Amount</tr>\n";
...
if($lastdate){
#	print "</table>";
#	print "<p><b>Count of Unmatched transactions in GL for ". cutzeroes($lastdate) . ":\t$dateglcount</b></p>";
	print "<h2>List of Unmatched transactions from CoreCard</h2>";
	print "<table><tr><td>Effective Date<td>Time<td>Card Number<td>Amount</tr>\n";
	 for my $ccrec (@ccunmatched){
		next unless goofydatematch($lastdate, $ccrec->{Transaction_Date});
